...
def _fetch_viash_pages() -> list:
    """Crawl guide/reference pages from viash.io and return as LlamaIndex Documents."""
    import re
    import time
    import requests
    ...
    sitemap_url = "https://viash.io/sitemap.xml"
    print("  Fetching viash.io sitemap...")
    r = requests.get(sitemap_url, timeout=15)
    r.raise_for_status()
    all_urls = re.findall(r"<loc>(.*?)</loc>", r.text)
    urls = [u for u in all_urls if any(x in u for x in ["/guide/", "/reference/", "/faq"])]
    # Exclude deprecated pages to keep index clean
    urls = [u for u in urls if "/deprecated/" not in u]
    print(f"  Found {len(urls)} pages to index")

    docs = []
    session = requests.Session()
    session.headers["User-Agent"] = "Mozilla/5.0 (viash-docs-indexer)"
    ...
    for i, url in enumerate(urls):
        try:
            ...
            main = soup.find("main") or soup.find("article") or soup.find("div", class_=re.compile(r"content|prose|doc"))
            text = (main or soup).get_text(separator="\n", strip=True)
            # Skip near-empty pages
            if len(text) < 100:
                continue
            docs.append(Document(text=text, metadata={"url": url, "source": "viash.io"}))
        except Exception as e:
            print(f"  Warning: failed to fetch {url}: {e}")
        if i % 10 == 9:
            print(f"  Fetched {i + 1}/{len(urls)} pages...")
            time.sleep(0.2)

    print(f"  Loaded {len(docs)} viash.io pages")
    return docs


def get_or_build_viash_index(use_cache: bool = True) -> VectorStoreIndex:
    """Build or load the viash.io docs index (fetched from viash.io/guide and /reference)."""
    _configure_embed_model()

    persist_dir = Path(__file__).parent.parent / ".index_cache" / "viash" if use_cache else None
    ...
    if persist_dir and persist_dir.exists():
        ...
        current_tag = _embed_model_tag()
        if cached_tag != current_tag:
            print(f"Embed model changed ({cached_tag} → {current_tag}), rebuilding viash index …")
            shutil.rmtree(persist_dir)
        else:
            print(f"Loading cached viash index from {persist_dir}")
            storage_context = StorageContext.from_defaults(persist_dir=str(persist_dir))
            return load_index_from_storage(storage_context)

    print("Building viash.io index (fetching from web)...")
    documents = _fetch_viash_pages()
    if not documents:
        raise RuntimeError("No viash.io pages could be fetched. Check your internet connection.")
    ...
    if persist_dir:
        ...
        index.storage_context.persist(persist_dir=str(persist_dir))
        (persist_dir / "embed_model.json").write_text(json.dumps({"model": _embed_model_tag()}))
        print(f"  Viash index persisted to {persist_dir}")
