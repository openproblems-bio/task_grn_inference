...
noise_types=("net" "weight" "direction" "sign")
...
append_entry() {
  ...
  cat >> "$param_local" << HERE
  - id: ${grn_name}_${dataset}_noise_${noise_type}_degree_${degree}
    ...
    for noise_type in "${noise_types[@]}"; do
      for degree in "${degrees[@]}"; do
        if [[ -f "${grn_models_folder_local}/${dataset}/${grn_name}.h5ad" ]]; then
          append_entry "$grn_name" "$dataset" "$noise_type" "$degree"
        fi
      done
    done
  done
...
HERE
  ...
  nextflow run . \
    -main-script  target/nextflow/workflows/run_permutation_analysis/main.nf \
    -profile docker \
    -with-trace \
