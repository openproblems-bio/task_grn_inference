...
def compute_spearman_for_group(X, groups, group):
    """Compute Spearman correlation for a single group."""
    mask = (groups == group)
    if np.sum(mask) >= 3:
        if not np.any(np.isnan(X[mask, :])):
...
def main(par):
    ...
    group_names.append("donor_id/plate_name/cell_type")
    ...
    for group_name, groups in zip(group_names, group_variables):
        ...
        # Compute correlation matrix for each group in parallel
        unique_groups = np.unique(groups)
        n_jobs = min(len(unique_groups), 10)
        
        print(f"Computing Spearman correlations for {len(unique_groups)} groups using {n_jobs} cores...")
        
        # Parallel computation of Spearman correlations
        results = Parallel(n_jobs=n_jobs)(
            delayed(compute_spearman_for_group)(X, groups, group) 
            for group in unique_groups
        )
        
        # Extract results
        S, C = [], []
        for stat, pval, group in results:
            if stat is not None:
                S.append(stat)
                C.append(pval)
