...
def main(par):
    ...
    for tf, row in edges_count.iterrows():
        ...
        consensus.append({'source':tf, 'theta': 0.25, 'value':int(np.quantile(row_nozero, 0.25))})
        consensus.append({'source':tf, 'theta': 0.75, 'value':int(np.quantile(row_nozero, 0.75))})
