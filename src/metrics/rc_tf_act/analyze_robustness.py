...
from util import process_links, shuffle_grn
...
def analyze_robustness(par):
    ...
    print("="*60)
    print("RC_TF_ACT Robustness Analysis")
    print("="*60)
...
def create_robustness_plot(df_results, output_dir):
    ...
    # Create subplots: 1 row × 3 columns (precision, balanced, recall)
    fig, axes = plt.subplots(1, 3, figsize=(18, 5))
    
    metrics = ['precision', 'balanced', 'recall']
    metric_labels = {
        'precision': 'Precision (Top 10 TFs, 10 edges each)',
        'balanced': 'Balanced (Top 100 TFs, 100 edges each)',
        'recall': 'Recall (Top 300 TFs, 1000 edges each)'
    }
    ...
    for col_idx, metric in enumerate(metrics):
        ...
        if metric_col in df_results.columns:
            for noise_type in noise_types:
                df_noise = df_results[df_results['noise_type'] == noise_type]
                ax.plot(df_noise['degree'], df_noise[metric_col], 
