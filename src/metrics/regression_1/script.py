...
parser.add_argument(
    "--evaluation_data", type=str, help="Path to the evaluation_data file"
)
...
try:
    ...
    meta = {"resources_dir": "src/metrics/regression_1/", "util_dir": "src/utils"}
    sys.path.append(meta["resources_dir"])
    sys.path.append(meta["util_dir"])

from util import verbose_print, process_links, binarize_weight
from helper import set_global_seed, cross_validation
...
if par["apply_skeleton"]:
    ...
    skeleton["link"] = (
        skeleton["source"].astype(str) + "_" + skeleton["target"].astype(str)
    )
...
assert all(column in net.columns for column in ["source", "target", "weight"]), (
    "Columns in the network should be source, target, weight"
)
...
# - formatize results
results = dict(
    r1_all=[results["r2score-aver-all"]], r1_grn=[results["r2score-aver-grn"]]
)
