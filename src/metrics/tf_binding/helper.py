...
def main(par):
    ...
    G = np.abs(G)
    G += np.random.uniform(0, 1e-20, size=G.shape)

    # Evaluate inferred GRN
    tf_binding_precision = precision_score(A.flatten(), (G != 0).flatten())
    tf_binding_recall = recall_score(A.flatten(), (G != 0).flatten())
