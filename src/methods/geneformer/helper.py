...
def efficient_melting(matrix, gene_names, symmetric=False):
    ...
    if symmetric:
        # For symmetric matrices, only take upper triangle
        i_indices, j_indices = np.triu_indices(n_genes, k=1)
    else:
        # For non-symmetric matrices, take all entries
        i_indices, j_indices = np.meshgrid(range(n_genes), range(n_genes), indexing='ij')
        i_indices = i_indices.flatten()
        j_indices = j_indices.flatten()
    
    # Create the melted dataframe
    melted_df = pd.DataFrame({
        'source': [gene_names[i] for i in i_indices],
        'target': [gene_names[j] for j in j_indices],
        'weight': matrix[i_indices, j_indices]
    })
    
    return melted_df


def tryParallelFunction(func, label, **kwargs):
    """Try parallel function execution with fallback"""
    try:
        func(nproc=os.cpu_count(), **kwargs)
    except Exception as e:
        print(f"{label} failed with error: {e}")
        print("Retrying with nproc=1")
        func(nproc=1, **kwargs)
...
def main(par):
    ...
    for i, cell_type in enumerate(adata.obs["cell_type"].unique()):
        ...
        subadata, net = compute_geneformer_network(
            ...
            gene_mapping_dict=gene_mapping_dict,
            token=token,
            model_dir=model_dir,
            tokenized_dir=tokenized_dir,
