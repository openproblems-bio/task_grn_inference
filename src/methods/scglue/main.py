...
def training(par):
    ...
    glue = scglue.models.fit_SCGLUE(
        ...
        fit_kws={"directory": f"{temp_dir}/glue"}
    )

    glue.save(f"{temp_dir}/glue.dill")
    ...
    rna.write(f"{temp_dir}/rna-emb.h5ad", compression="gzip")
    atac.write(f"{temp_dir}/atac-emb.h5ad", compression="gzip")
    nx.write_graphml(guidance, f"{temp_dir}/guidance.graphml.gz")
...
def cis_inference(par):
    ...
    atac = ad.read_h5ad(f"{temp_dir}/atac-emb.h5ad")
    guidance = nx.read_graphml(f"{temp_dir}/guidance.graphml.gz")
    ...
    scglue.genomics.Bed(atac.var).write_bed(f"{temp_dir}/peaks.bed", ncols=3)
    scglue.genomics.write_links(
        gene2peak,
        scglue.genomics.Bed(rna.var).strand_specific_start_site(),
        scglue.genomics.Bed(atac.var),
        f"{temp_dir}/gene2peak.links", keep_attrs=["score"]
    )
    ...
    command = (
        f"pyscenic grn {temp_dir}/rna.loom {temp_dir}/tfs.txt "
        f"-o {temp_dir}/draft_grn.csv --seed 0 --num_workers {num_workers} "
    ...
    scglue.genomics.write_scenic_feather(gene2tf_rank_glue, f"{temp_dir}/glue.genes_vs_tracks.rankings.feather")
    scglue.genomics.write_scenic_feather(gene2tf_rank_supp, f"{temp_dir}/supp.genes_vs_tracks.rankings.feather")
    ...
    ).to_csv(f"{temp_dir}/ctx_annotation.tsv", sep="\t", index=False)

    # Construct the command 
    #TODO: be sure that obs_id is in obs and name is in var
    command = (
        f" pyscenic ctx {temp_dir}/draft_grn.csv {temp_dir}/glue.genes_vs_tracks.rankings.feather "
        f" {temp_dir}/supp.genes_vs_tracks.rankings.feather  --annotations_fname {temp_dir}/ctx_annotation.tsv "
        f" --expression_mtx_fname {temp_dir}/rna.loom --output {temp_dir}/pruned_grn.csv "
        f" --rank_threshold 500 --min_genes 1  --num_workers {num_workers} "
        " --cell_id_attribute obs_id --gene_attribute name"
    )
