...
read_task_api <- function(path) {
  ...
  task_info <-
    tryCatch({
      read_and_merge_yaml(task_info_yaml, project_path)
    }, error = function(e) {
      stop("Failed to read task info yaml: ", e$message)
    })

  cli::cli_inform("Reading task authors")
  authors <-
    tryCatch({
      map_df(task_info$authors, function(aut) {
        aut$roles <- paste(aut$roles, collapse = ", ")
        list_as_tibble(aut)
      })
    }, error = function(e) {
      stop("Failed to read task authors: ", e$message)
    })

  cli::cli_inform("Reading component yamls")
  comp_yamls <- list.files(api_dir, pattern = "comp_.*\\.ya?ml", full.names = TRUE)
  comps <- map(comp_yamls, function(yaml) {
    tryCatch({
      read_comp_spec(yaml)
    }, error = function(e) {
      stop("Failed to read component yaml: ", e$message)
    })
  })
  ...
  files <- map(file_yamls, function(yaml) {
    tryCatch({
      read_file_spec(yaml)
    }, error = function(e) {
      stop("Failed to read file yaml: ", e$message)
    })
  })
  ...
  task_graph <- tryCatch({
    ...
    stop("Failed to create task graph: ", e$message)
  })
